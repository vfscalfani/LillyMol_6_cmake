                               ...
                               int len)
{
  ...
  cerr << "sizeof(regex_t) is " << sizeof (_compiled_regular_expression) << ", flags " << _regcomp_flags << '\n';
#endif
...
#ifdef DEBUG_SET_PATTERN
  cerr << "Pattern '" << s << "', re_nsub = " << _compiled_regular_expression.re_nsub << '\n';
  cerr << "Flags " << _regcomp_flags << '\n';
#endif
...
IW_grep_25_regex::_matches_save_subexpressions (const char * s, int len)
{
                     ...
                     _reg_eflags))
  {
    ...
    {
      cerr << "regmatch[" << i << "] from " << _regmatch[i].rm_so << " to " << _regmatch[i].rm_eo << '\n';
    }
#endif
...
#ifdef DEBUG_DOLLAR
  cerr << "Looking for dollar " << whichdollar << " in '" << _string_which_was_matched << "', range " << _regmatch[0].rm_so << " - " << _regmatch[0].rm_eo << '\n';
  cerr << "Match starts at " << rm.rm_so << " to " << rm.rm_eo << '\n';
#endif
