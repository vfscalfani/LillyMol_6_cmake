...
{
  if (name_rx && ! iwre2::RE2PartialMatch(m.name(), *name_rx)) {
    return 1;
  }
  ...
  if (cl.option_present('Y'))
  {
    ...
    while (cl.value('Y', y, i++))
    {
      ...
      else if (y.starts_with("chname=") || y.starts_with("CHNAME="))
      {
        ...
        y.remove_leading_chars(7);
        if (! iwre2::RE2Reset(change_name_rx, y)) {
        ...
        {
          cerr << "The chname regular expression must have subexpression(s) '" << change_name_rx->pattern() << "' invalid\n";
          return 3;
        }
