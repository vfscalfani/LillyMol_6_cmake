...
static Molecule_Output_Object * stream_for_oriented_molecules = NULL;
...
write_header (IWString_and_File_Descriptor & output)
{
  ...
  output << " sh_cigar1 sh_cigar13 sh_cigar23 sh_boxy sh_boxz sh_boxvol sh_sphere sh_nsphere sh_msphere sh_flat sh_nflat sh_mflat sh_tube sh_ntube sh_mtube";
  output << " sh_bow";
...
static int
do_compute_radius_of_gyration (Molecule & m,
                               const Atom ** atom,
                               coord_t & rg1,
                               coord_t & rg2)
{
...
static void
report_average_position (const Molecule & m,
                         const char * msg,
                         std::ostream & output)
{
...
static int
do_write_to_stream_for_oriented_molecules (Molecule & m,
                                           int sequence)
{
...
static void
do_rotate_into_xy_plane (Molecule & m,
...
// Now rotate so FURTHEST_ATOM is in the X/Y plane
  ...
  Coordinates yaxis(static_cast<coord_t>(0.0), static_cast<coord_t>(1.0), static_cast<coord_t>(0.0));
  ...
  Coordinates xaxis(static_cast<coord_t>(1.0), static_cast<coord_t>(0.0), static_cast<coord_t>(0.0));
...
{
  Coordinates zaxis(static_cast<coord_t>(0.0), static_cast<coord_t>(0.0), static_cast<coord_t>(1.0));
...
static int
is_heteroatom_pair (const Atom * const * atom,
                    int i1, 
                    int i2)
{
...
static coord_t c0 = static_cast<coord_t>(0.0);
...
//  Accumulator<float> _distance_to_bond_count_ratio;
  ...
  public:
    ...
    const int * process_these_atoms() const { return _process_these_atoms;}

    const Accumulator<coord_t> & inter_atomic_distance() const { return _inter_atomic_distance;}

    const Accumulator<coord_t> & inter_heteroatom_distance() const { return _inter_heteroatom_distance;}

//  Accumulator<coord_t> & distance_to_bond_count_ratio() { return _distance_to_bond_count_ratio;}
//  const Accumulator<coord_t> & distance_to_bond_count_ratio() const { return _distance_to_bond_count_ratio;}

    const vdw_radius_t * vdw() const { return _vdw;}

    int assign_vdw_radii (Molecule &, int);

    void determine_extremeties (const Molecule &, const Atom ** const);
    void do_rotate_to_longest_distance (Molecule & m, const Atom ** atom);
};
...
int
Shadow_Tmp::assign_vdw_radii (Molecule & m,
                              int vdw_type)
{
...
#ifdef DEBUG_PBF
//  cerr << "Atom " << i << " at (" << c.x() << ',' << c.y() << ',' << c.z() << ") dist " << d << endl;
#endif
  }
...
static void
assemble_A_matrix_and_B_vector (const Molecule & m,
                                TNT::Array2D<tnt_float_type> & a,
...
static bool
getBestFitPlane(const std::vector<Coordinates> &points,
...
static double
distanceFromAPlane(const Coordinates &pt,const std::vector<double> &plane, double denom){
  double numer=0.0;
...
static double PBFRD(Molecule & mol)
{
...
#ifdef ECHO_MATRICES
  cerr << "TNT PCA singular values " << singular_values[0] << " " << singular_values[1] << ' ' << singular_values[2] << endl;
#endif
...
//  cerr << " atom " << i << " x " << t2[i][0] << " y " << t2[i][1] << " z " << t2[i][2] << " sum " << pbf << endl;
  }
...
#ifdef DEBUG_PBF
  cerr << "Equation is A = " << eqa << " B = " << eqb << " C = " << eqc << " D = " << eqd << endl;
#endif
...
//cerr << "Valyes are " << s[0] << ' ' << s[1] << ' ' << s[2] << ", norm " << norm << ", dim " << s.dim() << endl;
...
//cerr << "Valyes are " << s[0] << ' ' << s[1] << ' ' << s[2] << " norm " << tmp << endl;

  return 1;
}

void
Shadow_Tmp::determine_extremeties (const Molecule & m,
                       const Atom ** atom)
{
  const auto matoms = m.natoms();
...
void
Shadow_Tmp::do_rotate_to_longest_distance (Molecule & m,
                                           const Atom ** atom)
{
...
// In order to enforce more uniform behaviour, canonicalise left and right. 
  ...
  if (verbose > 2)
  {
    ...
    cerr << m.name() << " extremity atoms " << left << " " << m.smarts_equivalent_for_atom(left) << ' ' << a->x() << ' ' << a->y() << ' ' << a->z() << endl;

    a = m.atomi(right);
    cerr << " and " << right << ' ' << m.smarts_equivalent_for_atom(right) << ' ' << a->x() << ' ' << a->y() << ' ' << a->z() << " dist " << maxd << endl;
  }
...
// Translate the molecule so that atom LEFT is at the origin
  ...
  assert (static_cast<coord_t>(0.0) == atom[left]->x() && static_cast<coord_t>(0.0) == atom[left]->y() && static_cast<coord_t>(0.0) == atom[left]->z());
...
// At this stage, the molecule should have LEFT at the origin and RIGHT somwhere along the X axis
  ...
  cerr << "left is " << left << " (" << atom[left]->x() << ',' << atom[left]->y() << "), right " << right << " (" << atom[right]->x() << ',' << atom[right]->y() << ")\n";
#endif
...
static void
do_rotate_to_longest_distance_end_queries (Molecule & m,
                                           const Atom ** atom,
                                           Shadow_Tmp & stmp)
{
...
static void
do_rotate_to_longest_distance (Molecule & m,
                               const Atom ** atom,
                               Shadow_Tmp & stmp)
{
  ...
  {
    cerr << "Warning, none of " << queries.number_elements() << " queries, match, default processing\n";
...
/*
  ...
  Remember that the shadow class does projections onto the YZ plane, so the order in which the values
  are computed might not necessarily correspond with the orientations here
*/
...
//  cerr << "Examining atom " << i << " (" << ai->x() << ',' << ai->y() << ',' << ai->z() << ")\n";
...
// Write the descriptors
  ...
  const Accumulator<coord_t> & inter_heteroatom_distance = stmp.inter_heteroatom_distance();
...
// M* numbers are mostly the distances from these ideal locations
  ...
  output.add(1.73205080756 - sqrt((1.0-amid/abig)*(1.0-amid/abig) + (1.0-amin/abig)*(1.0-amin/abig)));

  output.add(abig - amid);     // flat
  output.add((abig - amid) / abig);     // nflat
  output.add(1.73205080756 - sqrt((amid*amid)/(abig*abig) + (amin*amin)/(abig*abig)));  // mflat

  output.add(amid - amin);     // tube
  output.add((amid - amin) / abig);     // ntube
  output.add(1.73205080756 - sqrt((1.0-amid/abig)*(1.0-amid/abig) + (amin*amin)/(abig*abig)));  // mtube

  output.add(static_cast<float>(zz.average()));      // sh_bow, aka pbf

  output.add(static_cast<float>(fabs(zleft.average() - zright.average())));   // just abs diff between average Z values, sh_zanisotropy
                       ...
                       IWString_and_File_Descriptor & output)
{
  ...
  for (auto i = 0; i < n; ++i)
  {
    ...
    {
      const int c = static_cast<int>((v - shd_minval[i]) / (shd_maxval[i] - shd_minval[i]) * max_bit_count + 0.4999) + 1;
      sfc.hit_bit(i, c);
    }
  }
...
static int
do_output (const resizable_array<float> & shd,
           IWString_and_File_Descriptor & output)
{
  if (pbf_only)
  {
...
static int
do_output (const IWString & mname,
           const resizable_array<float> & shd,
           IWString_and_File_Descriptor & output)
{
...
static int
tshadow (data_source_and_type<Molecule> & input, 
         IWString_and_File_Descriptor & output)
{
...
static int
tshadow (const char * fname, int input_type, 
         IWString_and_File_Descriptor & output)
{
  if (0 == input_type)
  {
...
static int
tshadow_filter (iwstring_data_source & input,
                IWString_and_File_Descriptor & output)
{
...
tshadow (int argc, char ** argv)
{
  ...
  if (cl.option_present('P'))
  {
    ...
    if ("ext" == p)
    {
      ...
      if (verbose)
        cerr << "Alignment done by principal components, initial position by extremeties\n";
    }
    else if ("ave" == p)
    {
      position_by_extremeties = 0;
      if (verbose)
        cerr << "Alignment done by principal components, initial position averaged across origin\n";
    }
    else
    {
...
//  If nothing specified for what to write, write everything
    ...
    for (int i = 1; i <= 3; i++)
    {
      ...
      stream_for_oriented_molecules[i].add_output_type(SDF);
  ...
  if (verbose)
  {
    ...
    if (acc.n() > 0)
      cerr << "Shadow areas between " << acc.minval() << " and " << acc.maxval() << " average " << acc.average() << endl;
  }
  ...
  if (check_rdkit)
  {
    ...
    if (rdkit_diff.n() > 0)
    {
      cerr << " differences between " << static_cast<float>(rdkit_diff.minval()) << " and " << static_cast<float>(rdkit_diff.maxval());
      if (rdkit_diff.n() > 1)
  ...
  if (NULL != stream_for_oriented_molecules)
