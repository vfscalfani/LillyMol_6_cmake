...
tnass (Molecule & m)
{
  ...
  {
    if (verbose > 1)
      cerr << "Results from query " << i << " are " << results[i] << '\n';
...
tnass (int argc, char ** argv)
{
  ...
  if (cl.option_present('J'))     // will write fingerprints
  {
    ...
    if (cl.option_present('y'))
    {
      ...
      {
        cerr << "The -y option must be followed by a whole number between 2 and " << queries.number_elements() << '\n';
        usage(15);
      }
    }
    else
    ...
    if (verbose)
      cerr << "Query matches written as fingerprints of size " << default_fingerprint_nbits << '\n';
  }
  ...
  if (verbose)
  {
    ...
    cerr << "Over " << queries.searches_performed() << " optimisation " << queries.optimisation_level() << '\n';
    ...
    if (acc.n() > 1)
      cerr << "Average hits per molecule " << acc.average() << '\n';
  }
