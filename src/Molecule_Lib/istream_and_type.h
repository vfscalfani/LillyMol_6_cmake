...
#include "Foundational/iwcrex/iwcrex.h"
...
//  private functions
    ...
    int  _set_rx_for_input_type (std::unique_ptr<RE2> & rx) const;
...
template <typename T>
int
data_source_and_type<T>::_set_rx_for_input_type (std::unique_ptr<RE2> & rx) const
{
  switch (_input_type)
  {
    case MDL:
    case SDF:
      rx = std::make_unique<RE2>("^\\$\\$");
      break;
    ...
    case IWMTYPE_MRK:
      rx = std::make_unique<RE2>("^[ 0-9]{5} [ 0-9]{5} *$");
      // rx.set_pattern ("^[ 0-9]{5} [ 0-9]{5} *$");
      break;

    case PDB:
      rx = std::make_unique<RE2>("^END$");
      break;
