...
template <typename T>
int
assign_atom_types_za(Molecule & m,
                     T * invariant)
{
  const int matoms = m.natoms();

  if (std::is_same<T, int>::value)
    m.atomic_numbers(invariant);
  else
  {
    for (int i = 0; i < matoms; ++i)
    {
      invariant[i] = m.isotope(i);
    }
  }
...
static int
assign_atom_types_za(Molecule & m, int * invariant)
{
  const int matoms = m.natoms();
...
static int
assign_atom_types_za(Molecule & m, unsigned int * invariant)
{
  const int matoms = m.natoms();
...
assign_atom_types_z(Molecule & m, 
                    T * invariant)
{
  m.atomic_numbers(reinterpret_cast<atomic_number_t *>(invariant));
                              ...
                              T * atype)
{
  ...
  if (0 == two_connected_nitrogen_count.number_elements())
    return 0;
...
//  cerr << " expanded               '" << buffer << "'\n";
    ...
    else if (buffer.starts_with("write"))
    {
      ...
      _write_isotopically_labelled.add_output_type(SMI);
